...
def d2():
    exp_name = 'refactored_alpha_gfn_set_new_icml'
    ...
    for (sum, name) in zip(runs_pd["summary"], runs_pd["name"]):
        ...
        # exit()
        try:
            if exp_name == 'refactored_alpha_gfn_set_new_icml':
                if sum_dict['step'] != 9999:
                    continue
                if sum_dict['training_mode'] != 'online':
                    continue
...
def d3():
    exp_name = 'refactored_alpha_gfn_set_new_icml_fl0'
    ...
    for (sum, name) in zip(runs_pd["summary"], runs_pd["name"]):
        ...
        # exit()
        try:
            if exp_name == 'refactored_alpha_gfn_set_new_icml_fl0':
                if sum_dict['step'] != 9999:
                    continue
                if sum_dict['training_mode'] != 'online':
                    continue
